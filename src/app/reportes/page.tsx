...
const PDFButton = loadDynamic(
  () => import("@/components/ReportePDF").then((mod) => mod.PDFButton),
  { ssr: false }
);
...
export default function ReportesPage() {
  ...
  const obtenerDispositivos = async () => {
    try {
      ...
      setDispositivos(
        Array.isArray(dispositivosConCosto) ? dispositivosConCosto : []
      );
    } catch (error) {
  ...
  const promedioDiario =
    dispositivos.length > 0 ? consumoTotal / dispositivos.length : 0;
  const picoMaximo = dispositivos.reduce(
    (max, d) => (d.consumo > max ? d.consumo : max),
    0
  );
  const costoEstimado = consumoTotal * precioKWh;
  const datosLinea = dispositivos.map((d) => ({
    fecha: d.nombre,
    consumo: d.consumo,
  }));
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-10 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white">
      <h1 className="text-3xl font-bold mb-8 text-center">
        Reportes y Análisis Energético
      </h1>
      ...
      <div className="flex flex-wrap gap-4 mb-6 justify-center">
        ...
        {/* ✅ Componente PDF cargado dinámicamente */}
        <PDFButton
